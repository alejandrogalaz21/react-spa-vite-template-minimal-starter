...
export const socketStatusEpic: AppEpic = (action$, _state$): Observable<AppAction> =>
  merge(
    /**
     * 🟢 SOCKET CONNECTED
     */
     ...
     */
    action$.pipe(
      filter((action): action is AppAction => action.type === socketConnected.type),
      map(() => flushQueue())
    )
  );
